...
CREATE OR REPLACE FUNCTION fn_novo_sal(p_perct in number, p_id_func in number)
    RETURN number IS
    v_sal_novo number;
BEGIN
    SELECT (a.salary*(p_perct/100)) + a.salary
    INTO v_sal_novo
    FROM hr.employees a WHERE a.employee_id=p_id_func;
...
select a.first_name, a.salary as salario_antigo, fn_novo_sal(50,a.EMPLOYEE_ID) as novo_salario
from HR.employees a;
