...
-- AVG Retorna a média dos valores em um grupo. Valores nulos são ignorados
select cast(AVG(populacao) as decimal(12,2)) as media from senso
where ano='2014';
...
select estado, cast(avg(populacao) as decimal(12,2)) as media, ano
from senso
where estado like 'P%'
group by estado, ano
order by ano asc;
...
-- NVL Substitui o ISNULL com, NVL(campo, 'valor se nulo')
SELECT NVL(a.regiao, 'Valor Nulo') as regiao , cast(AVG(a.POPULACAO) as decimal(12,2)) as media 
	FROM senso a
    WHERE a.ano='2014'
    group by a.regiao
    ORDER BY 2 DESC;
...
-- COUNT  POR ESTADO
SELECT a.estado,COUNT(a.nome_mun) FROM senso a
where ano='2014'
GROUP BY a.estado
ORDER BY 2 desc;
...
-- POR ESTADO
SELECT a.estado,
	   cast(avg(a.populacao) as decimal(12,2))media_pop,
	   min(a.populacao)minimo_pop,
	   max(a.populacao)maximo_pop,
	   sum(a.populacao)total_pop,
	   COUNT(*) qtd_cidades
...
-- apresentar nome da cidade com menor populacao de cada estados
select a.estado, a.nome_mun, a.populacao
from (
...
select a.estado,
    sum(a.populacao) as pop_senso_2011,
    sum(b.populacao) as pop_senso_2012,
    cast((sum(b.populacao)*100)/(sum(a.populacao))-100 as decimal(12,2)) as dif_percentual
from senso a
inner join senso b
    on a.estado = b.estado
where a.ano = 2011
...
-- GROUP BY IGNORA OS VALORES NULOS
SELECT a.estado,COUNT(*) FROM senso a
where ano='2014'
GROUP BY a.estado
order by 2 desc;

-- COM ROLLUP RETORNA OS VALORES NULOS
SELECT a.estado,COUNT(*) FROM senso a
where ano='2014'
