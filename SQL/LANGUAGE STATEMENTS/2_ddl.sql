...
-- DDL CRIACAO DE INDEX
 CREATE INDEX ix_func2   ON funcionario(cidade ASC,pais DESC);
...
as 
select * from aluno.funcionarios;
...
begin
    ...
    DBMS_OUTPUT.put_line('Retorno: ' || v_num1 ||' * ' || v_num1 || ' = '|| v_num2);
end;

create or replace procedure proc_resto(v_num1 number, v_num2 number)
is
v_num3 number;
begin
    v_num3:= mod(v_num1, v_num2);
    DBMS_OUTPUT.put_line('Modulo de ' || v_num1 || ' e ' || v_num2 || ' é ' || v_num3);
end;
...
-- CRIANDO TABELA TEMPORARIA BK EM TABELA TEMPORARIA, AO REALIZAR COMMIT OS DADOS DESAPARECEM
