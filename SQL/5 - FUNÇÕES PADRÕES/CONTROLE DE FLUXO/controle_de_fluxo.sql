...
select a.estado, sum(a.populacao) soma_pop_br,
case when sum(a.populacao) < 10000000 then 'BAIXO'
     when sum(a.populacao) < 30000000 then 'MEDIO'
     else 'ALTO' end
from senso a
group by estado
order by 2 desc;
...
--função NVL trata os valores nulos
SELECT A.FIRST_NAME,A.COMMISSION_PCT,NVL(A.COMMISSION_PCT,0)
 FROM HR.EMPLOYEES A;