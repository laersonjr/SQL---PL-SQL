...
DECLARE
    v_palavra_recebida varchar2(100);
    v_palavra_criptografada varchar2(100);
    v_palavra_retorno varchar2(100);
    
    BEGIN
    v_palavra_recebida:= 'PALAVRA TESTE';
    v_palavra_criptografada:= utl_encode.text_encode(v_palavra_recebida,'WE8ISO8859P1', UTL_ENCODE.BASE64);
    dbms_output.put_line('PALAVRA RECEBIDA CRIPTOGRAFADA FORMATO WE8ISO8859P1: ' || v_palavra_criptografada);
    
    v_palavra_criptografada:= utl_encode.text_encode(v_palavra_recebida,'AL32UTF8', UTL_ENCODE.BASE64);
    dbms_output.put_line('PALAVRA RECEBIDA CRIPTOGRAFADA FORMATO AL32UTF8: ' || v_palavra_criptografada);
    
    v_palavra_retorno:= utl_encode.text_decode(v_palavra_criptografada, 'WE8ISO8859P1', UTL_ENCODE.BASE64);
    dbms_output.put_line('RETORNANDO VALOR DA PALAVRA CRIPTOGRAFADA: ' || v_palavra_retorno);
END;
...
--exemplo 2   
   ...
   DECLARE
   v_input varchar2(100):= 'Curso Oracle';
   v_input_criptografado varchar2(100);
    BEGIN
    v_input_criptografado:= rawtohex(dbms_obfuscation_toolkit.md5(input => utl_raw.cast_to_raw(v_input)));
    dbms_output.put_line('O valor hash para "' || v_input || '" é "' || nvl(v_input_criptografado, '') || '".');
    END;
--exemplo 3
